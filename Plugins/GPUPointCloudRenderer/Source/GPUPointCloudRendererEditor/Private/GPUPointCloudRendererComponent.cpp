...
//}
void UGPUPointCloudRendererComponent::SetDynamicProperties(float splatSize) {
	mSplatSize = splatSize;
}
...
void UGPUPointCloudRendererComponent::SetInputAndConvert1(TArray<FLinearColor> &pointPositions, TArray<FColor> &pointColors) {
	...
	if (pointPositions.Num() != pointColors.Num())
		UE_LOG(GPUPointCloudRenderer, Warning, TEXT("The number of point positions doesn't match the number of point colors: Points: %d, Colors: %d"), pointPositions.Num(), pointColors.Num());
...
void UGPUPointCloudRendererComponent::CreateStreamingBaseMesh(int32 pointCount)
{
	...
	int32 pointsPerAxis = FMath::CeilToInt(FMath::Sqrt((float)pointCount));
...
void UGPUPointCloudRendererComponent::BuildTriangleStack(TArray<FCustomMeshTriangle> &triangles, const int32 &pointCount)
{
	...
	const float a = 1.0f;				// side lenght
	const float sqrt3 = FMath::Sqrt(3.f);
	const float r = sqrt3 / 6 * a;	// radius of inscribed circle
...
void UGPUPointCloudRendererComponent::UpdateShaderProperties()
{
	if (!mPointCloudMaterial) {
		UE_LOG(GPUPointCloudRenderer, Warning, TEXT("Point Cloud Material is null!"));
		return;
	}
